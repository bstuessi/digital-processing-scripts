...
find "$directory" -type f -exec stat -f "%m" {} \; > dates.txt
...
min_date_formatted=$(date -r $min_date '+%Y-%m-%d')
max_date_formatted=$(date -r $max_date '+%Y-%m-%d')
avg_date_formatted=$(date -r $average '+%Y-%m-%d')
